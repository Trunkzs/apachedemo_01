pipeline {
  ...
  stages {
    ...
    stage('Run Jmeter test') {
      steps {
        sh 'docker exec alpine bash -c "$JMETER -n -t /opt/github/$JOB_NAME/SummaryReport.jmx -l /opt/result/TestResult-$BUILD_NUMBER.csv -e -o /opt/reports/$BUILD_NUMBER -R172.27.0.3"'   
    }
    }
