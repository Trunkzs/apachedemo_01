pipeline {
  ...
  stages {
    ...
    stage('Run Jmeter test') {
      steps {
        sh 'docker exec jmeter-master bash -c "/opt/apache-jmeter-5.6.2/bin/jmeter -n -t /opt/github/$JOB_NAME/SummaryReport.jmx -l /opt/result/TestResult-$BUILD_NUMBER.csv -e -o /opt/reports/$BUILD_NUMBER"'   
    }
    }
